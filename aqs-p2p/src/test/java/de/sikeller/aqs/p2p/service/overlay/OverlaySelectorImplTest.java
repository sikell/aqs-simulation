...
class OverlaySelectorImplTest {
  ...
  void selectDoesNotThrowAndReturnsVehiclesWhenRelevant() {
    ...
    var peers = List.of(new NodeDescriptor("vehicle-2", NodeRole.VEHICLE), new NodeDescriptor("client-1", NodeRole.CLIENT));
    var selection = sel.select("VEHICLE_POSITION", peers);
    assertNotNull(selection);
    // selection peers should be subset of input peers
    assertTrue(peers.containsAll(selection.peers()) || selection.peers().containsAll(peers) || !selection.peers().isEmpty());
  }
}
