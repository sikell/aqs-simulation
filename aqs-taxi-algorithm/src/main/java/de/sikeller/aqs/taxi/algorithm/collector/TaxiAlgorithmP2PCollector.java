...
public class TaxiAlgorithmP2PCollector extends AbstractTaxiAlgorithm implements P2PStatusProvider {
  ...
  private final Map<String, String> previousP2PSystemProperties = new HashMap<>();
  ...
  private final Map<String, RoamingTelemetry> roamingTelemetryByVehicleNodeId =
  ...
  public void init(World world) {
    ...
    roamingTelemetryByVehicleNodeId.clear();
  ...
  private void syncRemoteVehicleStates(World world) {
    if (clientService == null || world == null || taxiNameToVehicleNodeId.isEmpty()) {
      return;
    }
    Map<String, String> config = p2pStateConfigPayload();
    for (Taxi taxi : world.getTaxis()) {
      ...
      clientService.sendVehicleState(
          vehicleNodeId,
          taxi.isEmpty(),
          taxi.getPosition().getX(),
          taxi.getPosition().getY(),
          stepCounter,
          world.getSize().getMaxX(),
          world.getSize().getMaxY(),
          resolveSpawnScenario().name(),
          config);
    }
  }

  private Map<String, String> p2pStateConfigPayload() {
  ...
  private void putP2PProperty(Map<String, String> payload, String... keys) {
    for (String key : keys) {
      String value = P2PRunContext.getProperty(key);
      if (value != null) {
  ...
  private int applyCommittedAssignments(World world, Collection<Client> waitingClients) {
    ...
    for (AssignmentCandidate candidate : candidates) {
      ...
      announceWinner(
          pending.requestId(), pending.committedVehicleNodeId(), clientPos.getX(), clientPos.getY());
      refreshStatus(EVENT_ASSIGNED_PREFIX + pending.requestId());
      applied++;
    }
    assignedClients.forEach(runtimeState::removeByClient);
    return applied;
  }

  private void applyVehicleDispatchConfig() {
    String strategy = resolveVehicleSelectionStrategy();
    setP2PProperty(P2PSystemProperties.VEHICLE_OPEN_REQUEST_STRATEGY, strategy);
    setP2PProperty(
        P2PSystemProperties.VEHICLE_COMMIT_LEASE_TICKS,
    ...
    setP2PProperty(VEHICLE_IDLE_ROAMING_STRATEGY_PROPERTY, resolveIdleRoamingStrategy());
  }

  private String resolveIdleRoamingStrategy() {
    String configured =
        P2PRunContext.getProperty(
            VEHICLE_IDLE_ROAMING_STRATEGY_PROPERTY, IDLE_ROAMING_STRATEGY_RANDOM);
  ...
  private String resolveVehicleSelectionStrategy() {
    String configured = P2PRunContext.getProperty(P2PSystemProperties.VEHICLE_OPEN_REQUEST_STRATEGY, "");
    if (configured != null && !configured.isBlank()) {
  ...
  private void handleIncoming(World world, List<P2PMessage> messages) {
    for (P2PMessage message : messages) {
      ...
      if (P2PTopics.VEHICLE_ROAMING.equals(topic)) {
        roamingTelemetryByVehicleNodeId.put(message.senderId(), RoamingTelemetry.parse(payload));
        continue;
      }
      String vehicleNodeId = payload.getOrDefault(P2PPayloadKeys.VEHICLE, message.senderId());
      String requestId = message.requestId();
      if (requestId == null || requestId.isBlank()) {
        requestId = payload.get(P2PPayloadKeys.REQUEST_ID);
      }

      TaxiCollectorRuntimeState.PendingRequest pending = runtimeState.getByRequestId(requestId);
      if (pending == null) {
        continue;
      }
      ...
      if (P2PTopics.RIDE_COMMIT.equals(message.topic())) {
        ...
        String taxiName =
            vehicleNodeToTaxiName.getOrDefault(
                vehicleNodeId, payload.getOrDefault(P2PPayloadKeys.TAXI_NAME, vehicleNodeId));
        if (commitHandler == null || world == null) {
          runtimeState.markCommittedIfOpen(pending.requestId(), vehicleNodeId);
        } else {
          commitHandler.handleCommit(vehicleNodeId, taxiName, pending, world);
        }
      }
    }
  }
  ...
  private void handleDirectCommit(String requestId, String vehicleNodeId, String taxiName) {
    ...
    if (!runtimeState.markCommittedIfOpen(requestId, vehicleNodeId)) {
      return;
    }
    runtimeState.registerTaxiKnowledge(vehicleNodeId, pending.clientName());
  }
  ...
  public Map<String, String> getP2PStatus() {
    ...
    status.put(
        ...
        P2PRunContext.getProperty(P2PSystemProperties.OVERLAY_SHORTCUT_STRATEGY, "kleinberg"));
    ...
    status.put(
        ...
        P2PRunContext.getProperty(P2PSystemProperties.OVERLAY_MAX_NEIGHBORS, "100"));
    status.put(
        STATUS_OVERLAY_SHORTCUTS,
        P2PRunContext.getProperty(P2PSystemProperties.OVERLAY_SHORTCUTS, DEFAULT_OVERLAY_SHORTCUTS));
    status.put(
  ...
  public Map<String, Set<String>> getTaxiKnowledgeByClientIds() {
    Set<String> activeClientNames = runtimeState.activeClientNames();
  ...
  public Map<String, int[]> getPageRankHqPositions() {
    ...
    if (!isEmbeddedSimulationMode()) {
      roamingTelemetryByVehicleNodeId.forEach(
          (vehicleNodeId, telemetry) -> {
            String taxiName = vehicleNodeToTaxiName.get(vehicleNodeId);
            int[] pos = telemetry.positionFor(strategy);
            if (taxiName != null && pos != null) {
  ...
  public P2PNetworkSnapshot getP2PNetworkSnapshot() {
    ...
    return networkSnapshotBuilder.build(
        localNodeId,
        clientService.descriptor().role().name(),
        network.peers(),
        topologyViews,
        isEmbeddedSimulationMode() ? Map.of() : vehicleNodeToTaxiName);
  }
  ...
  private void applyOverlayConfig(Map<String, Integer> config) {
    ...
    setP2PProperty(P2PSystemProperties.OVERLAY_MIN_NEIGHBORS, String.valueOf(minNeighbors));
    setP2PProperty(P2PSystemProperties.OVERLAY_MAX_NEIGHBORS, String.valueOf(maxNeighbors));
    setP2PProperty(P2PSystemProperties.OVERLAY_SHORTCUTS, String.valueOf(shortcuts));
    setP2PProperty(
        P2PSystemProperties.OVERLAY_POSITION_TTL_TICKS, String.valueOf(positionTtlTicks));
    setP2PProperty(P2PSystemProperties.OVERLAY_MAX_DISTANCE, String.valueOf(maxDistance));
    if (P2PRunContext.getProperty(P2PSystemProperties.OVERLAY_SHORTCUT_STRATEGY) == null) {
  ...
  private void setP2PProperty(String key, String value) {
    if (!P2PRunContext.isActive()) {
      previousP2PSystemProperties.putIfAbsent(key, System.getProperty(key));
    }
    P2PRunContext.setProperty(key, value);
  }

  private void restoreP2PSystemProperties() {
    if (P2PRunContext.isActive()) {
      previousP2PSystemProperties.clear();
      return;
    }
    previousP2PSystemProperties.forEach(
        (key, value) -> {
          if (value == null) {
  ...
  private boolean isTopologyScanEnabled() {
    return parameters.getOrDefault(P2P_TOPOLOGY_SCAN_ENABLED, isEmbeddedSimulationMode() ? 0 : 1)
        == 1;
  }
  ...
  private void announceWinner(
      String requestId, String winnerVehicleNodeId, int pickupX, int pickupY) {
    if (requestId == null || requestId.isBlank()) {
      return;
    }
    ...
    if (clientService != null) {
      clientService.announceWinner(requestId, winnerVehicleNodeId, pickupX, pickupY);
    }
  }
  ...
  private record RoamingTelemetry(int[] hq, int[] revisit, int[] avgTotal) {
    private static RoamingTelemetry parse(Map<String, String> payload) {
      return new RoamingTelemetry(
          point(payload, P2PPayloadKeys.HQ_X, P2PPayloadKeys.HQ_Y),
          point(payload, P2PPayloadKeys.REVISIT_X, P2PPayloadKeys.REVISIT_Y),
          point(payload, P2PPayloadKeys.AVG_TOTAL_X, P2PPayloadKeys.AVG_TOTAL_Y));
    }

    private static int[] point(Map<String, String> payload, String xKey, String yKey) {
      String x = payload.get(xKey);
      String y = payload.get(yKey);
      if (x == null || y == null) {
        return null;
      }
