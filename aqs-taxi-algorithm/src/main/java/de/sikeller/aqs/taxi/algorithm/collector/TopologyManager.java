...
/**
 * Small helper that encapsulates topology scan handling and delegates storage/side-effects back to
 * the caller via callbacks. Keeps a minimal dependency surface (suppliers + callbacks) so the
 * owning collector can remain authoritative for topology storage.
 */
class TopologyManager {
  ...
  public record TopologyData(
      String role, Set<String> neighborIds, Set<String> shortcutNeighborIds) {
    public TopologyData(String role, Set<String> neighborIds, Set<String> shortcutNeighborIds) {
      ...
      this.shortcutNeighborIds =
          shortcutNeighborIds == null ? Set.of() : Set.copyOf(shortcutNeighborIds);
    }
  }
  ...
  public void requestScanIfDue(boolean force) {
    ...
    int period =
        Math.max(1, params.getOrDefault(PARAM_TOPOLOGY_SCAN_TICKS, DEFAULT_TOPOLOGY_SCAN_TICKS));
  ...
  public void upsertLocalTopologyView() {
    ...
    upsertCallback.accept(
        localNodeId, new TopologyData(clientNode.descriptor().role().name(), Set.of(), Set.of()));
  }
